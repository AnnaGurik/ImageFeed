...
final class ImagesListService {
    ...
    private func makeRequestPage(page: Int) -> URLRequest {
        guard
            let url = URL(string: "/photos?page=\(page)", relativeTo: AuthConfiguration.standard.defaultBaseURL),
    ...
    private func makeRequestLike(photoId: String, isLike: Bool) -> URLRequest {
        guard
            let url = URL(string: "/photos/\(photoId)/like", relativeTo: AuthConfiguration.standard.defaultBaseURL),
            let token = OAuth2TokenStorage().token
        else { fatalError() }
